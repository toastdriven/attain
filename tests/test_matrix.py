...
def test_small_matrix_get(small_matrix):
    ...
    assert small_matrix.get("world", "world", 1.0) == 1.0
...
def test_medium_matrix_get(medium_matrix):
    ...
    assert medium_matrix.get("b", "b", 1.0) == 0.2
    assert medium_matrix.get("c", "a", 1.0) == 0.3
...
def test_repr(medium_matrix):
    assert repr(medium_matrix) == "<SparseSquareMatrix: 4>"
